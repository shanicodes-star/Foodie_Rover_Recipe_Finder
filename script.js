...
function updateLikeDislikeDisplay(recipeId) {
  ...
  const likeBtn = modal.find('.likeBtn');
  const dislikeBtn = modal.find('.dislikeBtn');
  likeBtn.data('id', recipeId);
  dislikeBtn.data('id', recipeId);

  likeBtn.find('.likeCount').text(recipeLikes[recipeId] || 0);
  dislikeBtn.find('.dislikeCount').text(recipeDislikes[recipeId] || 0);

  const userLiked = localStorage.getItem(`liked_${recipeId}`) === 'true';
  const userDisliked = localStorage.getItem(`disliked_${recipeId}`) === 'true';
  const loggedIn = isUserLoggedIn();

  if (!loggedIn || userLiked || userDisliked) {
...
function showVoteConfirmation(type) {
  const message = type === 'liked' ? 'Recipe liked! 👍' : 'Recipe disliked! 👎';
  const popup = $(`<div class="vote-confirmation">${message}</div>`);
  $('body').append(popup);
  ...
  popup.css({
    ...
    left: '50%',
    transform: 'translate(-50%, -50%)',
    background: type === 'liked' ? '#d4edda' : '#f8d7da',
    color: type === 'liked' ? '#155724' : '#721c24',
    border: `2px solid ${type === 'liked' ? '#c3e6cb' : '#f5c6cb'}`,
    padding: '15px 30px',
    borderRadius: '10px',
    fontWeight: '600',
    boxShadow: '0 4px 15px rgba(0,0,0,0.2)',
...
$(document).ready(function () {
  ...
  $(document).on('click', '.view-btn', function () {
    ...
    $.getJSON(apiBase + "lookup.php?i=" + id, function (data) {
      ...
      for (let i = 1; i <= 20; i++) {
        ...
        if (ing && ing.trim() !== "") ingredientsHTML += `<li>${ing} - ${measure}</li>`;
      }
      $('#ingredientList').html(ingredientsHTML);
  ...
  $(document).on('click', '.likeBtn', function () {
    ...
    if (localStorage.getItem(`liked_${recipeId}`) === 'true' || localStorage.getItem(`disliked_${recipeId}`) === 'true') return;
    ...
    updateLikeDislikeDisplay(recipeId);
    showVoteConfirmation('liked');
  });
  ...
  $(document).on('click', '.dislikeBtn', function () {
    ...
    localStorage.setItem(`disliked_${recipeId}`, 'true');

    updateLikeDislikeDisplay(recipeId);
    showVoteConfirmation('disliked');
  });
  ...
  $(document).on("click", "#submitReview", function () {
    ...
    const recipeId = $('.likeBtn').data("id");
...
// =================== LOGIN SYSTEM ===================
var savedEmail = "test@example.com";
var savedPassword = "12345";
...
$('#translateBtn').click(function () {
  ...
  if (isUrdu) {
    ...
    if (!translatedInstructions) translatedInstructions = translateToUrdu(originalInstructions);

    // Translate ingredients
    if (!translatedIngredients) {
      let plainIngredients = $('#ingredientList li').map(function(){ return $(this).text(); }).get().join("\n");
      let translatedIngText = translateToUrdu(plainIngredients);
      translatedIngredients = translatedIngText.split("\n").map(item => `<li>${item}</li>`).join("");
    }

    // Show Urdu
    $('#instructionsText').html(translatedInstructions);
    $('#ingredientList').html(translatedIngredients);
    $(this).text('Translate to English');
  } else {
    // Show original
    $('#instructionsText').html(originalInstructions);
